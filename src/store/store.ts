...
import { PageCommand } from './page/command/page-command';
...
export class Store {
	...
	/**
	 * The most recent undone page commands (user operations) to provide a redo feature.
	 * Note that operations that close or open a page clear this buffer.
	 * The last command in the list is the most recent undone.
	 */
	@MobX.observable private redoBuffer: PageCommand[] = [];
	...
	/**
	 * The most recent page commands (user operations) to provide an undo feature.
	 * Note that operations that close or open a page clear this buffer.
	 * The last command in the list is the most recent executed one.
	 */
	@MobX.observable private undoBuffer: PageCommand[] = [];
	...
	/**
	 * Executes a page command (user operation) and registers it as undoable command.
	 * @param command The command to execute and register.
	 */
	public execute(command: PageCommand): void {
		if (command.execute()) {
	...
	/**
	 * Returns whether there is a page comment (user operation) to redo.
	 * @return Whether there is a page comment (user operation) to redo.
	 * @see redo
	 */
	...
	/**
	 * Returns whether there is a page comment (user operation) to undo.
	 * @return Whether there is a page comment (user operation) to undo.
	 * @see undo
	 */
	 ...
	 */
	public redo(): boolean {
		const command: PageCommand | undefined = this.redoBuffer.pop();
		if (!command) {
			return false;
	 ...
	 */
	public undo(): boolean {
		const command: PageCommand | undefined = this.undoBuffer.pop();
		if (!command) {
			return false;
